...
import sys
import pm4py
import prod.discover as discover
import prod.discover_mr as discover_mr
import prod.discover_ta as discover_ta
...
if __name__ == '__main__':
    ...
    try:
        ...
        if (event_log.endswith('.xes') or event_log.endswith('.XES')):
            ...
            if log is None:
                raise RuntimeError("Log file not set")
        else:
        ...
        if (pattern_path != ''):
            ...
            print("concatenated_traces = ",concatenated_traces)
            print("concatenated_timestamps = ",concatenated_timestamps)
        ...
        if (input_pattern_seq != ''):
            print("Info: Performing abstraction steps " +
                  "based (TA/MR sets) patterns...")
            # perform abstraction and transformation
            if (pattern_path != ''):
